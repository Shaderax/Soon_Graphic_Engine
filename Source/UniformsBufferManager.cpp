...
namespace Soon
{
	...
	void UniformsBufferManager::RecreateUniforms( void )
	{
		uint32_t offset = 0;
		for (uint32_t index = 0; index < m_Uniforms.size(); index++)
		{
			for ( uint32_t elemId = 0 ; elemId < m_NumElements ; elemId++ )
			{
				std::vector<VkDescriptorSet> descriptorSets;
				descriptorSets = GraphicsInstance::GetInstance()->CreateDescriptorSets(m_Uniforms[index]._size, m_Uniforms[index]._binding, offset + m_SizeUniqueUniformBlock + (m_SizeUniformBlock * elemId), _descriptorSetLayout[m_Uniforms[index]._set], m_GpuBuffer.data(), 0);				
				for ( uint32_t descriptorId = 0 ; descriptorId < descriptorSets.size() ; descriptorId++ )
				{
					// TODO: UniformTexture
					m_DescriptorSets[descriptorId][m_Uniforms.size() * elemId + index] = descriptorSets[descriptorId];
				}
...
//		}
		...
		m_NumElements = (m_NumElements < idMat) ? idMat : m_NumElements;
	}
	...
	std::vector<VkDescriptorSet>& UniformsBufferManager::GetDescriptorSet( uint32_t image, uint32_t idMat )
	{
