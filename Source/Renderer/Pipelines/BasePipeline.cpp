...
namespace Soon
{
    ...
    void BasePipeline::DestroyAllUniforms(void)
    {
        ...
        {
            for (UniformSets &bu : _uniforms[index]._us)
            {
                for (uint32_t idChain = 0; idChain < GraphicsInstance::GetInstance()->GetSwapChainSize(); idChain++)
                {
                    vkDestroyBuffer(device, bu._uniformRender.buffer[idChain], nullptr);
                    vkFreeMemory(device, bu._uniformRender.bufferMemory[idChain], nullptr);
                }
            }
        }
    }
                ...
                if (_uniforms[index]._name == name.substr(0, pos))
                {
                    for (uint32_t member = 0; member < _uniforms[index]._members.size(); member++)
                    {
                        if (varName == _uniforms[index]._members[member]._name)
                        {
                            ...
                            vkMapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[0], 0, _uniforms[index]._size, 0, &data);
                            memcpy(data, &value, _uniforms[index]._size);
                            vkUnmapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[0]);

                            vkMapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[1], 0, _uniforms[index]._size, 0, &data);
                            memcpy(data, &value, _uniforms[index]._size);
                            vkUnmapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[1]);

                            vkMapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[2], 0, _uniforms[index]._size, 0, &data);
                            memcpy(data, &value, _uniforms[index]._size);
                            vkUnmapMemory(device, _uniforms[index]._us[id]._uniformRender.bufferMemory[2]);
                        }
                    }
                    return;
                }
